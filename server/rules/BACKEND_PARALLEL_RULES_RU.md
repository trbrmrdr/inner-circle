...
VK использует только `VK_GROUP_ID` как положительный числовой ID сообщества.
Отдельный owner env не вводим: если VK API требует `owner_id` стены, сервер вычисляет
его внутри `VkPublisher` как `-VK_GROUP_ID`.
...
`SETTINGS` использует колонки `key`, `value`, `description`, `value_ru`.
Сервер читает только `value`. `value_ru` нужен для русской версии prompt/value и
человеческого редактирования. Если `value` пустой или ключа нет, сервер обязан
брать default текущего env/кода и не падать.

DeepSeek управляется двумя уровнями:

- env `DEEPSEEK_ENABLED` - техническое разрешение сервиса на конкретном сервере;
- Google `SETTINGS.deepseek.enabled` - оперативный переключатель из таблицы.

DeepSeek работает только когда оба значения включены и есть API key. Prompt-ы
храним по платформам, начиная с `deepseek.telegram.prompt`: `value` - рабочий
...
Для VK и Telegram приватные Google Drive файлы сначала скачиваются на текущий
сервер, затем готовятся в платформенной папке. Для VK фото нормализуются в JPEG,
видео - в MP4/H.264/AAC без добавления черных полей.
...
DeepSeek не публикует посты. Он только подготавливает текст. Если `DEEPSEEK_ENABLED=false`, `deepseek.enabled=false` или DeepSeek упал, автопостинг использует исходный текст из таблицы.
