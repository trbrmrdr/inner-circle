...
- `text` - текст поста;
- `media_ids` - список ID из `MEDIA`, например `IMG_078;VID_0003;IMG_101`;
...
- новые `media_id` создаются как `IMG_001`, `IMG_002`, `VID_0001`, `VID_0002`;
- старые `M0001`, `image001`, `vid001` нужны только до миграции и не должны появляться у новых файлов;
...
Одноразовые правки данных Google Sheets не добавлять в Apps Script. Для
миграций, массового переименования, исправления строк и похожих задач писать
локальный скрипт под конкретную задачу, запускать его через Google Sheets API с
service account и после выполнения убирать одноразовые команды из постоянного
`package.json`. В Apps Script оставлять только постоянные функции таблицы и
Google Drive.
